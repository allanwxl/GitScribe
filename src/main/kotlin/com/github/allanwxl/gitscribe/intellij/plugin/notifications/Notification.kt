...
import com.github.allanwxl.gitscribe.intellij.plugin.GitScribeBundle.openPluginSettings
...
data class Notification(
    ...
    companion object {
        ...
        fun star() = Notification(
            message = """
                Finding GitScribe useful? Show your support 💖 and ⭐ the repository 🙏.
            """.trimIndent(),
            actions = setOf(
...
data class NotificationAction(val title: String, val run: (dismiss: () -> Unit) -> Unit) {
    companion object {
        ...
        fun openRepository(onComplete: () -> Unit) = NotificationAction(message("actions.sure-take-me-there")) { dismiss ->
            GitScribeBundle.openRepository()
            dismiss()
            onComplete()
        }
