...
import com.github.allanwxl.gitscribe.intellij.plugin.settings.AppSettings2.LocaleConverter
...
@State(
        ...
        storages = [Storage("GitScribe.xml")]
)
@Service(Service.Level.PROJECT)
class ProjectSettings : PersistentStateComponent<ProjectSettings?> {

    companion object {
        const val SERVICE_NAME = "com.github.allanwxl.gitscribe.intellij.plugin.settings.ProjectSettings"
    }
    ...
    fun isPathExcluded(path: String): Boolean {
        return GitScribeUtils.matchesGlobs(path, projectExclusions)
    }
